...
async function main() {
  ...
  if (!targetDir || !fs.existsSync(targetDir)) {
    console.error(
      "사용법: node scripts/batchAnalyze.js <대상 폴더> [출력.jsonl]",
    );
    process.exit(1);
  }
  ...
  console.log(
    `위험 발견 파일    : ${filesWithFindings}개 (${files.length ? Math.round((filesWithFindings / files.length) * 100) : 0}%)`,
  );
  console.log(`총 findings     : ${totalFindings}건`);
  console.log(
    `위험도 분포      : 높음 ${scoreBuckets["높음"]} / 중간 ${scoreBuckets["중간"]} / 낮음 ${scoreBuckets["낮음"]} / 안전 ${scoreBuckets["안전"]}`,
  );
