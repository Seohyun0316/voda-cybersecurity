# 🛡️ VibeSafe {{VERSION}} 배포 파일

이 폴더만 전달하면 VibeSafe Extension과 로컬 분석 백엔드를 설치할 수
있습니다.

## 📦 포함 파일

- `VibeSafe-{{VERSION}}.vsix`: VS Code Extension 설치 파일
- `VibeSafe-backend-{{VERSION}}.zip`: 로컬 분석 백엔드 압축 파일
...
3. `VibeSafe-{{VERSION}}.vsix`를 설치하고 VS Code를 다시 로드합니다.
